...
def test_generate_sql_filter_with_source_alias_preserves_alias_in_where():
    session_id = setup_session_with_data()
    ...
    tree = {
        ...
        "enabled": True,
        "commands": [
            {"id": "c1", "type": "source", "config": {"mainTable": "users", "alias": "u"}},
            {
                "id": "c2",
                "type": "filter",
                "config": {"field": "u.role", "operator": "=", "value": "user", "valueType": "raw"}
            }
        ],
        "children": []
    }
    ...
    assert "WHERE u.role = 'user'" in sql

def test_generate_sql_filter_data_source_with_qualified_field_keeps_alias():
    session_id = setup_session_with_data()
