...
def build_tree(target_cmd: dict, source_alias: str | None = None):
    base_tree = {
        "id": "root",
        "type": "operation",
        "name": "Root",
        "enabled": True,
        "commands": [
            {"id": "c1", "type": "source", "config": {"mainTable": "users", **({"alias": source_alias} if source_alias else {})}},
            {
                "id": "c2",
                "type": "group",
                "config": {
...
def main():
    ...
    for idx, item in enumerate(commands, start=1):
        ...
        tree = build_tree(deepcopy(item["cmd"]), item.get("source_alias"))
